...
void SteeringController::my_clever_steering_algorithm() {
    ...
    // vector pointing from odom x-y to desired x-y
    pos_err_xy_vec_ = des_xy_vec_ - odom_xy_vec_; 
    
    //signed scalar lateral offset error; if positive, then desired state is to the left of odom
    lateral_err = n_vec.dot(pos_err_xy_vec_);
    
    // progress error: if positive, then we are behind schedule
    trip_dist_err = t_vec.dot(pos_err_xy_vec_); 
    
    // if positive, should rotate +omega to align with desired heading
    ...
    // suitable for plotting w/ rqt_plot
    steering_errs_publisher_.publish(steering_errs_); 
    
    //Initially, keep the desired state velocity and omega
    controller_speed = des_state_vel_;
    controller_omega = des_state_omega_;
    
    //Add the trip distance error to the controller speed when
    //the trip distance error is outside of certian tolerance values.
    ...
    //Account for lateral error
    controller_omega += K_LAT * lateral_err;
    
    // saturate omega command at specified limits
    ...
    // copy the twist command into twist2 message
    twist_cmd2_.twist = twist_cmd_; 
    
    // look up the time and put it in the header
...
int main(int argc, char** argv) 
{
    ...
    while (ros::ok()) {
    	...
    	// compute and publish twist commands and cmd_vel and cmd_vel_stamped
